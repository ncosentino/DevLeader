# Thread Pattern Example - Threading Patterns and Background Thread Management in C#
...
Legacy .NET Framework 2.0 console application demonstrating manual threading patterns in C#. Implements IThreadRunner interface with two concrete implementations: SingleThreadRunner (manages a single background thread using Thread, AutoResetEvent, and lock-based synchronization) and GroupedThreadRunner (manages multiple threads in a thread group). The interactive console app lets users start/stop thread runners to observe background thread behavior.

This predates modern async/await patterns and serves as a reference for understanding low-level thread management in .NET. By examining how threads are created, synchronized, and stopped manually, developers can gain a deeper appreciation for the concurrency abstractions available in modern C# and .NET.
...
### Running the Project
Open in Visual Studio, build and run. Use the interactive console to start/stop thread runners.
