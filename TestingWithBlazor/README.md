# TestingWithBlazor - Blazor Component Testing with bUnit and xUnit
...
This project demonstrates Blazor component testing patterns using bUnit and xUnit. The `ExampleBlazorApp` project is a standard Blazor Server application featuring a `WeatherForecastService` and the default Blazor template pages, providing a realistic target for unit testing.

The `ExampleBlazorApp.Tests` project uses bUnit 1.24.10, xUnit 2.5.3, and Moq 4.20.69 to show how to unit test Blazor components with dependency mocking. It demonstrates how to render components in a test context, inject mock services, and assert against the rendered markup — key skills for building reliable Blazor applications.

Built on .NET 7, this solution serves as a practical starting point for developers looking to add automated testing to their Blazor Server projects using the bUnit testing library.
